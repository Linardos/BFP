...
def select_largest_obj(img_bin, lab_val=255, fill_holes=False, 
    ...
    n_labels, img_labeled, lab_stats, _ = \
        cv2.connectedComponentsWithStats(img_bin, connectivity=4, 
                                            ltype=cv2.CV_32S)
    ...
    if smooth_boundary:
        ...
        largest_mask = cv2.dilate(largest_mask, cv2.getStructuringElement(cv2.MORPH_CROSS,(5,5)), iterations=2)
...
def preprocessed_mini_mias_image(img_path, roi_abnormality, case_id, output_folder, save_masked=False, plot=False, opening=False):

    png_img_path = os.path.join(output_folder, case_id)
    png_mask_path = None

    scaled = cv2.imread(img_path, cv2.IMREAD_UNCHANGED)
    if roi_abnormality:
        png_mask_path = os.path.join(output_folder, case_id + '_mask')
        mask = np.zeros(scaled.shape, dtype=np.uint8)
        (x, y, radius) = roi_abnormality
        x1 = x - radius
        y1 = scaled.shape[1] - y - radius
        x2 = x + radius
        y2 = scaled.shape[1] - y + radius
        # mask_norm = cv2.circle(scaled, (x, scaled.shape[1] - y), radius, (255,0,0), 2)
        mask_norm = cv2.rectangle(mask, (x1, y1), (x2, y2), (255,255,255), -1)
        if plot:
            plt.figure()
    ...
    img_suppr, breast_mask, cropped_coordinates = suppress_artifacts(scaled, global_threshold=.05, fill_holes=False, 
                        smooth_boundary=True, kernel_size=15, plot=plot, opening=opening)
    ...
    if save_masked:
        # Make output folders
        masked_img_folder = os.path.join(str(output_folder),'masked')
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1:
        ...
        config_file = Path(
            '/home/lidia/source/BreastCancer/src/configs/preprocess_mini_mias.yaml')
    ...
    # apply_opening = ['mdb005', 'mdb035', 'mdb041', 'mdb045']
    selected_case = None
    # selected_case = 'mdb183'
    with tqdm(total=info.shape[0]) as pbar:
        for case in info.iterrows():
            if (selected_case is None) or (case[0] == selected_case):
                # if case[0] in apply_opening:
                #     opening = True
                # else:
                #     opening = False
                opening = True
                img_scan_path = os.path.join(dataset_path, case[0] + '.pgm')
                if case[1]['Center_x'] != '':
                    roi_abnormality = (int(case[1]['Center_x']), int(case[1]['Center_y']), int(case[1]['Radius']))
                else:
                    roi_abnormality = None
                img_norm_path, msk_norm_path = preprocessed_mini_mias_image(img_scan_path, roi_abnormality, case[0], 
