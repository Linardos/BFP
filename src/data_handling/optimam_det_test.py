...
def save_dataset_split_to_COCO(info_csv, dataset_path, cropped_to_breast, pathologies, manufacturer, client_ids,
    ...
    if json_out:
        ...
        df_bboxes = clients_save_COCO_annotations(clients_selected, pathologies, json_file, fit_to_breast=fit_to_breast,
                                ...
                                df_bboxes=df_bboxes)
        df_bboxes.to_csv(os.path.join(output_path, f'OPTIMAM_train_split_{class_name}_rescaled_bboxes.csv'), index=False)
                ...
                for annotation in annot_elem:
                    ...
                    if df_bboxes is not None:
                        ...
                        df_bboxes.loc[bbox_count] = [image.id] + [img_elem['file_name']] + [annotation['bbox'][2]*image.pixel_spacing] + \
                                                    ...
                                                    [image.view] + [image.status] + [image.age]
                        bbox_count += 1
                    if False:
...
def get_bbox_size_distribution(info_csv, dataset_path, detection, cropped_to_breast,
    ...
    total_info = {'total': 0,
                 ...
                 #'status': {'Malignant': 0}, # Philips
                 'view': {'CC': 0, 'MLO' :0},
                 'age': {'<50': 0, '50 to 60': 0, '60 to 70': 0, '>70': 0}}
    diameter_info = {'<1.0cm': {'total':0, 'status':{'Benign': 0, 'Malignant': 0}, 'view': {'CC': 0, 'MLO' :0}, 'age': {'<50': 0, '50 to 60': 0, '60 to 70': 0, '>70': 0}},
                '1.0-1.5cm': {'total':0, 'status':{'Benign': 0, 'Malignant': 0}, 'view': {'CC': 0, 'MLO' :0}, 'age': {'<50': 0, '50 to 60': 0, '60 to 70': 0, '>70': 0}},
                '1.5-2.0cm': {'total':0, 'status':{'Benign': 0, 'Malignant': 0}, 'view': {'CC': 0, 'MLO' :0}, 'age': {'<50': 0, '50 to 60': 0, '60 to 70': 0, '>70': 0}},
                '2.0-3.0cm': {'total':0, 'status':{'Benign': 0, 'Malignant': 0}, 'view': {'CC': 0, 'MLO' :0}, 'age': {'<50': 0, '50 to 60': 0, '60 to 70': 0, '>70': 0}},
                '3.0-4.0cm': {'total':0, 'status':{'Benign': 0, 'Malignant': 0}, 'view': {'CC': 0, 'MLO' :0}, 'age': {'<50': 0, '50 to 60': 0, '60 to 70': 0, '>70': 0}},
                '>4.0cm': {'total':0, 'status':{'Benign': 0, 'Malignant': 0}, 'view': {'CC': 0, 'MLO' :0}, 'age': {'<50': 0, '50 to 60': 0, '60 to 70': 0, '>70': 0}}}
    ...
    df_out = pd.DataFrame(columns=['mass_diameter', 'total', 'Benign', 'Malignant', 'MLO', 'CC', '<50', '50 to 60', '50 to 60', '>70'])
    #df_out = pd.DataFrame(columns=['mass_diameter', 'total', 'Malignant', 'MLO', 'CC', '<50', '50 to 60', '50 to 60', '>70']) #Philips
    row_ctr = 0
...
if __name__ == '__main__':
    ...
    out_csv_files = [os.path.join(output_path, f'OPTIMAM_HOLOGIC_{class_name}_all_bboxes.csv'),
                os.path.join(output_path, f'OPTIMAM_SIEMENS_{class_name}_all_bboxes.csv'),
                os.path.join(output_path, f'OPTIMAM_GE_MEDICAL_SYSTEMS_{class_name}_all_bboxes.csv'),
                os.path.join(output_path, f'OPTIMAM_Philips_Digital_Mammography_Sweden_AB_{class_name}_all_bboxes.csv')]
    for image_list, out_csv in zip(image_lists, out_csv_files):
        ...
        # diameter_info.to_csv(os.path.join(out_bbox_folder, f'OPTIMAM_Hologic_bboxes_size_info.csv'), index=False)
