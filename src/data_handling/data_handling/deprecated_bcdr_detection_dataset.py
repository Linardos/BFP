...
import matplotlib.pyplot as plt
from src.preprocessing.resize_image import *
...
MANUFACTURERS = [''] #TODO Info not found
SITES = ['Centro Hospitalar São João at University of Porto (FMUP-HSJ)']
PIXEL_SIZE = [0.07, 0.07] #mm # INFO not verified
RESOLUTION = 2e14 #14 bit resultion TIFFs (TIF files: 8bit)
            ...
            for patient_id, patient_group in unique_patient_id_df:
                ...
                for study_id, patient_study_group in unique_study_id_df:
                    ...
                    for series_id, series_group in unique_series_df:
                        ...
                        row_client = get_client(client_name, series_group.density.values[0])
                        ...
                        for image_name, image_group in unique_image_id_df:
                            ...
                            for idx_mark, image in enumerate(image_group.itertuples()):
                                if detection and image.lesion_x2 == '': # Check if Annotation is available
                                    break
                                if init_image:
                                    new_image.set_image_id('/'.join([dataset_path_i.split('/')[-1], image.scan_path]))
...
def save_images_ids(dataset, txt_out_file):
    # Save image list in txt
    image_ids = []
    for client in dataset:
        if pathologies:
...
def save_dataset_to_COCO(info_csv, dataset_path, cropped_scans, pathologies,
    ...
    for client in clients_selected:
        ...
        for image in images:
            ...
            for annotation in image.annotations:
                #if 'mass' in annotation.pathologies:
                if any(item in annotation.pathologies for item in pathologies):
...
if __name__ == '__main__':
    ...
    save_dataset_to_COCO(info_csv, dataset_path, cropped_scans, pathologies,
